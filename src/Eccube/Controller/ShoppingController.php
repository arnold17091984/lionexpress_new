...
class ShoppingController extends AbstractShoppingController
{
     ...
     *
     * @Route("/shopping/checkout", name="shopping_checkout", methods={"GET","POST"})
     * @Template("Shopping/confirm.twig")
     */
    ...
    {        
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            //return $this->redirect('https://lavy.shop/ssl_settlement.php?IP_CODE='.$request->get('IP_CODE').'&payment_code='.$request->get('payment_code').'&price='.$request->get('price').'&sendid='.$request->get('sendid').'&sendpoint='.$request->get('sendpoint').'&telno='.$request->get('telno').'&product_name='.$request->get('product_name').'&email='.$request->get('email').'&family='.$request->get('family').'&site_code='.$request->get('site_code').'&cookie='.$request->get('cookie').'&post='.$request->get('post').'&address='.$request->get('address').'&product_info='.$request->get('product_info').'&name='.$request->get('name'));
...
return $this->redirectToRoute('shopping_complete');
     ...
     * 決済プラグインによる決済処理および注文の確定処理を行います.
     *
     * @Route("/checkout_LAPI_complete", name="checkout_LAPI_complete", methods={"GET", "POST"})
     * 
     */
    public function lapi_complete(Request $request)
    {
        if($request->get('rel')==1){
            $preOrderId = $request->get('sendid');
            ...
            if ($request) {
                ...
                $response = new RedirectResponse('https://lion-express.com/shopping/complete');   
                return $response;
            }        
        }
        if ($this->orderHelper->isLoginRequired()) {
     ...
     * 注文処理を行う.
     *
     * 決済プラグインによる決済処理および注定処理を行います.
     *
     * @Route("/checkout_LAPI_agree", name="checkout_LAPI_agree", methods={"GET", "POST"})
     * 
     */
    public function lapi_agree(Request $request)
    {
        if($request){
            if($request->get('rel')==1)
            {
     ...
     * 決済プラグインによる決済処理および注文の確定処理を行います.
     *
     * @Route("/checkout_parm", name="checkout_param", methods={"GET", "POST"})
     * 
     */
    public function param_check(Request $request)
    {       
        $response= new Response();
        $response->setContent('OK');        
        $response->setStatusCode(Response::HTTP_OK);   
     ...
     * 購入エラー画面.
     *
     * @Route("/checkout_exception", name="checkout_exception", methods={"GET"}) 
     */

     public function spa_check_error()
     {
          // 受注とカートのずれを合わせるため, カートのPurchaseFlowをコールする.
          return $this->redirectToRoute('shopping_error');
     }
     ...
     * キックバック検証
     *
     * @Route("/checkout_parm_sucess", name="checkout_parm_sucess", methods={"GET", "POST"}) 
     */

     public function param_check_sucess(Request $request)
     {  
        $response = new Response();
        $response->setContent('OK');
        $response->setStatusCode(Response::HTTP_OK);
     ...
     * 決済プラグインによる決済処理および注文の確定処理を行います.
     *
     * @Route("/checkout_mobile", name="checkout_mobile", methods={"GET", "POST"})
     * 
     */
     ...
     * 決済プラグインによる決済処理および注文の確定処理を行います.
     *
     * @Route("/checkout_recieve", name="checkout_recieve", methods={"GET", "POST"})
     * 
     */
    public function mobile_checkout(Request $request)
    {
        // ログイン状態のチェック.
        
        
        // 受注の存在チェック
        $cart_key = $this->cartService->getCarts();
        
        $preOrderId = $cart_key[0]->getPreOrderId();
        $Order = $this->orderHelper->getPurchaseProcessingOrder($preOrderId);

        
        // $Order = $this->orderRepository->findOneBy(['pre_order_id' => $cart_key[0]->getPreOrderId()]);
     ...
     * 決済プラグインによる決済処理および注文の確定処理を行います.
     *
     * @Route("/checkout_recieve_exception", name="checkout_recieve_exception", methods={"GET", "POST"})
     * 
     */
     ...
     * 決済プラグインによる決済処理および注文の確定処理を行います.
     *
     * @Route("/tc_checkout", name="tc_checkout", methods={"GET", "POST"})
     * 
     */
    public function tc_pay_checkout(Request $request)
    {
        $preOrderId = $this->cartService->getPreOrderId();
        $Order = $this->orderHelper->getPurchaseProcessingOrder($preOrderId);
     ...
     * 決済プラグインによる決済処理および注文の確定処理を行います.
     *
     * @Route("/tc_kick_checkout", name="checkout_mobile", methods={"GET", "POST"})
     * 
     */
    public function tc_kick_check(Request $request)
    {    
        
        if ($request->get('sendid') && $request->get('rel')==1 ) {
            $preOrderId = $request->get('sendid');
            ...
            if($Order->getOrderStatus()->getId()==1){
                ...
                $response->setStatusCode(Response::HTTP_OK);                   
                return $response;
            }else if($Order->getOrderStatus()->getId()!=1){
     ...
     * 購入エラー画面.
     *
     * @Route("/tc_checkout_error", name="tc_checkout_error", methods={"GET"}) 
     */

     public function tc_check_error()
     {
          // 受注とカートのずれを合わせるため, カートのPurchaseFlowをコールする.
          return $this->redirectToRoute('shopping_error');
     }   

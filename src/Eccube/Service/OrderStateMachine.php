...
class OrderStateMachine implements EventSubscriberInterface
{
    ...
    public function updatePaymentDate(Event $event)
    {
        ...
        if (isset($_COOKIE['ref'])) {
            ...
            curl_setopt($req_ref, CURLOPT_URL,"https://affiliate.kusurisu.com/api/sellWebhook");
            curl_setopt($req_ref, CURLOPT_POST, 1);
            curl_setopt($req_ref, CURLOPT_POSTFIELDS,"ref=".$refferalCode."&orderId=".$Order->getId());

            curl_setopt($req_ref, CURLOPT_RETURNTRANSFER, true);

            $response = curl_exec($req_ref);

...
class CustomerRankDiscountProcessor implements DiscountProcessor
{
    ...
    protected $customerRankRepository;


    public function __construct(EntityManagerInterface $entityManager, CustomerRankRepository $customerRankRepository)
    {
        ...
        if(!is_null($CustomerRank)){
            if($CustomerRank->getDiscountValue() > 0){
                ...
                if($addDiscount){
                    ...
                    $discountValue = $CustomerRank->getDiscountValue();
                    if($discountValue > $itemHolder->getTotal()){
                        $discountValue = $itemHolder->getTotal();
                    }

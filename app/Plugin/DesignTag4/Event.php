...
class Event implements EventSubscriberInterface
{
    ...
    public static function getSubscribedEvents()
    {
        return [
            ...
            'Block/topics2.twig' =>'onProductListTwig2',
            'Block/new_item.twig' =>'onProductListTwig3',
            'Block/eyecatch.twig' =>'onProductListTwig4',
        ];
    }
...
//        $event->setSource($this->twigService->getSource());
    ...
    public function onProductListTwig2(TemplateEvent $event)
    {
        $DesignTags = $this->tagRepository->findBy(['show_product_list_flg' => true]);
        $event->setParameter('DesignTags', $DesignTags);
        $event->addAsset('@DesignTag4/tag_list_css2.twig');
    }

    public function onProductListTwig3(TemplateEvent $event)
    {
        $DesignTags = $this->tagRepository->findBy(['show_product_list_flg' => true]);
        $event->setParameter('DesignTags', $DesignTags);
        $event->addAsset('@DesignTag4/tag_list_css3.twig');
    }

    public function onProductListTwig4(TemplateEvent $event)
    {
        $DesignTags = $this->tagRepository->findBy(['show_product_list_flg' => true]);
        $event->setParameter('DesignTags', $DesignTags);
        $event->addAsset('@DesignTag4/tag_list_css4.twig');
    }
}

...
class LineIntegrationAdminController extends AbstractController
{
    ...
    public function search(Request $request, PaginatorInterface $paginator)
    {
        ...
        // POSTされたなら条件で検索する
        // ページング処理の場合はセッションの情報をもとに検索する。
        if ('POST' === $request->getMethod() || !empty($request->get('page_no')) ) {
            $pageCount = $this->eccubeConfig->get('eccube_search_pmax');
            $pageMaxis = $this->pageMaxRepository->findAll();

            $searchForm->handleRequest($request);
            $searchData = array();
            if(!empty($request->get('page_no'))){
                //ページング処理の場合はセッションを取得する
                $searchData = $this->session->get('plugin.line_integration.search');

                if(!empty($searchData)){
                    //性別が含まれる場合。もっとスマートなやり方はあるのか・・・？
                    $searchDataSex = $this->session->get('plugin.line_integration.search_sex');
                    if (strpos($searchDataSex, ',') === false) {
                        $searchData['sex'] = $this->entityManager->getRepository('Eccube\Entity\Master\Sex')->findBy(['id' => $searchDataSex]);
                    } elseif (!empty($searchDataSex)) {
                        $searchData['sex'] = $this->entityManager->getRepository('Eccube\Entity\Master\Sex')->findAll(['id' => $searchDataSex]);
                    } else {
                        $searchData['sex'] = $this->entityManager->getRepository('Eccube\Entity\Master\Sex')->findAll(['id' => 0]);
                    }

                    if (!empty($searchData['pref'])) {
                        $searchData['pref'] = $this->entityManager->getRepository('Eccube\Entity\Master\Pref')->find($searchData['pref']->getId());
                    }

                    $searchForm->setData($searchData);
                    if (is_numeric($request->get('page_no'))) {
                        $searchData['pageno'] = (int)$request->get('page_no');
                    }
                }
                ...
                // 性別の条件をセッションに保管 → ページングに利用
                foreach ($_POST['line_search']['sex'] as $key => $value) {
                    $searchDataSex[] = $value;
                }
                if (empty($searchDataSex)) {
                    $this->session->remove('plugin.line_integration.search_sex');
                } else {
                    $this->session->set('plugin.line_integration.search_sex', implode(',', $searchDataSex));
                }
            }
